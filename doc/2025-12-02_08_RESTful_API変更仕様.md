...
Recordings APIをRESTful設計に変更し、ステートレスなAPI設計を実現する。
...
現在のRecordings APIは、サーバー側の`state.getPattern()`に依存している：

```typescript
// 現在の実装
controlApi.get("/recordings/:filename", async (c) => {
...
| URIでリソース識別 | `/recordings/:filename` | パターンが不明なため一意に特定できない |
