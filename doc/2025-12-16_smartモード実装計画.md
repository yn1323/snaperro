...
describe("handleSmart", () => {
  ...
  describe("mockがある場合", () => {
    it("既存mockを返す（実サーバーにアクセスしない）", async () => {
      ...
      await fs.writeFile(
        path.join(BASE_DIR, TEST_SCENARIO, "users_001.json"),
...
smartモードもrecordモードと同様にシナリオ選択が必要なので、条件を更新:

```typescript
const handleModeClick = (targetMode: Mode) => {
  // record と smart はシナリオ選択が必要
  if ((targetMode === "record" || targetMode === "smart") && !currentScenario) {
    toaster.create({
      type: "warning",
