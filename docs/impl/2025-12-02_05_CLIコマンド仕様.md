# snaperro CLIコマンド仕様

## 概要

snaperroはCLIツールとして提供され、`npx snaperro` コマンドで操作します。

---

## インストール

```bash
npm install -D snaperro
# または
pnpm add -D snaperro
# または
yarn add -D snaperro
```

---

## コマンド一覧

| コマンド | 説明 |
|---------|------|
| `npx snaperro init` | プロジェクトを初期化 |
| `npx snaperro start` | サーバーを起動 |
| `npx snaperro postman` | Postmanコレクションを出力 |

---

## npx snaperro init

プロジェクトを初期化します。

### 実行

```bash
npx snaperro init
```

### 処理内容

1. `.snaperro/` ディレクトリを作成
2. `.snaperro/files/` ディレクトリを作成
3. `.snaperro/state.json` を作成（初期状態）
4. `snaperro.config.ts` のテンプレートを作成（存在しない場合）
5. `.gitignore` に `.snaperro/` を追加（存在しない場合）

### 出力例

```
$ npx snaperro init

🐕 snaperro init

✓ Created .snaperro/
✓ Created .snaperro/files/
✓ Created .snaperro/state.json
✓ Created snaperro.config.ts
✓ Added .snaperro/ to .gitignore

Done! Run 'npx snaperro start' to start the server.
```

### 生成されるファイル

#### .snaperro/state.json

```json
{
  "mode": "proxy",
  "currentScenario": null
}
```

#### snaperro.config.ts

```typescript
import { defineConfig } from 'snaperro'

export default defineConfig({
  port: 3333,
  filesDir: '.snaperro/files',

  apis: {
    // API定義をここに追加
    // example: {
    //   name: "Example API",
    //   target: "https://api.example.com",
    //   headers: {
    //     "X-Api-Key": process.env.EXAMPLE_API_KEY!,
    //   },
    //   routes: [
    //     "/api/example",
    //     "/api/example/:id",
    //   ],
    // },
  },
})
```

### エラーケース

#### 既に初期化済みの場合

```
$ npx snaperro init

🐕 snaperro init

⚠ .snaperro/ already exists
? Do you want to reinitialize? (y/N)
```

---

## npx snaperro start

サーバーを起動します。

### 実行

```bash
npx snaperro start
```

### オプション

| オプション | 説明 | デフォルト |
|-----------|------|----------|
| `--port`, `-p` | ポート番号を指定 | 設定ファイルの値 または `3333` |

### 使用例

```bash
# デフォルトポートで起動
npx snaperro start

# ポート4000で起動
npx snaperro start --port 4000
npx snaperro start -p 4000
```

### 処理内容

1. `snaperro.config.ts` を読み込み
2. 設定を検証
3. `.snaperro/state.json` から前回の状態を復元
4. サーバーを起動

### 出力例

```
$ npx snaperro start

🐕 snaperro server starting...

  Config: snaperro.config.ts
  Port: 3333
  Mode: mock (restored from previous session)
  Scenario: 正常系フル (restored from previous session)

  APIs loaded:
  - userService (5 routes)
  - orderService (4 routes)
  - authService (4 routes)

✓ Server running at http://localhost:3333

  GUI: http://localhost:3333
  Control API: http://localhost:3333/__snaperro__/

Press Ctrl+C to stop
```

### 状態の復元

サーバー起動時に `.snaperro/state.json` を読み込み、前回のモードとシナリオを復元します。

```json
// .snaperro/state.json
{
  "mode": "mock",
  "currentScenario": "正常系フル"
}
```

復元される項目：
- **mode**: 前回のモード（proxy/record/mock）
- **currentScenario**: 前回選択していたシナリオ

### エラーケース

#### 設定ファイルがない場合

```
$ npx snaperro start

Error: Configuration file not found.
Run 'npx snaperro init' first.
```

#### 設定が不正な場合

```
$ npx snaperro start

Error: Invalid configuration
- apis.userService.target: Invalid URL
- apis.orderService.routes: Cannot be empty

Please fix snaperro.config.ts and try again.
```

#### ポートが使用中の場合

```
$ npx snaperro start

Error: Port 3333 is already in use.
Try a different port: npx snaperro start --port 4000
```

---

## npx snaperro postman

Postmanコレクションを標準出力に出力します。

### 実行

```bash
npx snaperro postman
```

### 処理内容

1. `debug/postman-collection.json` を読み込み
2. JSON形式で標準出力に出力

### 使用例

```bash
# 標準出力に表示
npx snaperro postman

# ファイルに保存
npx snaperro postman > snaperro-collection.json

# Postmanにインポート
# 1. 出力をファイルに保存
# 2. Postman → Import → Upload Files で読み込み
```

### 出力例

```json
{
  "info": {
    "_postman_id": "snaperro-api-collection",
    "name": "snaperro API Collection",
    "description": "snaperro (スナペーロ) - GUI付きモックプロキシサーバーのAPI集",
    "schema": "https://schema.getpostman.com/json/collection/v2.1.0/collection.json"
  },
  "variable": [
    {
      "key": "baseUrl",
      "value": "http://localhost:3333",
      "type": "string"
    }
  ],
  "item": [...]
}
```

### エラーケース

#### コレクションファイルがない場合

```
$ npx snaperro postman

Error: debug/postman-collection.json が見つかりません
```

---

## 環境変数

### SNAPERRO_PORT

ポート番号を環境変数で指定できます。

```bash
SNAPERRO_PORT=4000 npx snaperro start
```

優先順位：
1. `--port` オプション
2. `SNAPERRO_PORT` 環境変数
3. 設定ファイルの `port`
4. デフォルト値 `3333`

---

## npm scripts での使用

### package.json

```json
{
  "scripts": {
    "mock": "snaperro start",
    "mock:init": "snaperro init"
  }
}
```

### 使用例

```bash
npm run mock
npm run mock:init
```

---

## 関連ドキュメント

- [概要](./2025-12-02_01_概要.md)
- [サーバー仕様](./2025-12-02_02_サーバー仕様.md)
- [内部API仕様](./2025-12-02_03_内部API仕様.md)
- [設定ファイル仕様](./2025-12-02_04_設定ファイル仕様.md)
- [JSONスキーマ仕様](./2025-12-02_06_JSONスキーマ仕様.md)
