...
現在のFiles APIは、サーバー側の`state.getScenario()`に依存している：

```typescript
// 現在の実装
controlApi.get("/files/:filename", async (c) => {
  const scenario = state.getScenario();  // サーバー側stateに依存
...
| URIでリソース識別 | `/files/:filename` | シナリオが不明なため一意に特定できない |

### 改善後

URLにシナリオ名を含めることで、リクエストのみでリソースを一意に特定可能にする。
