...
**新規作成:** `client/src/hooks/useScenarioNavigation.ts`

App.tsxから抽出:
- `currentFolder`
- `handleFolderSelect`, `handleFolderBack`, `handleScenarioSelect`
...
**変更対象:**
- `CreateScenarioModal.tsx` - CreateModal を使用
...
| `client/src/hooks/useScenarioNavigation.ts` | ナビゲーションフック |
...
| `client/src/components/CreateScenarioModal.tsx` | 共通モーダル使用 |
