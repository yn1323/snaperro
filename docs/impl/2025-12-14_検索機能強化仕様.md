...
**目標**: 検索APIエンドポイントを追加し、シナリオ内ファイルの全文検索機能を実装

#### 修正ファイル
| ファイル | 変更内容 |
|---------|---------|
| `server/core/storage.ts` | `searchScenarioFiles` 関数追加 |
| `server/handlers/control-api.ts` | `/scenarios/:scenario/files/search` エンドポイント追加 |
...
controlApi.get("/scenarios/:scenario/files/search", async (c) => {
  ...
  if (!(await storage.scenarioExists(scenario))) {
    return c.json({ error: "Not found", resource: "scenario", name: scenario }, 404);
  }
...
```bash
# サーバー起動後、curlでテスト
curl "http://localhost:3000/__snaperro__/scenarios/folder%2Fscenario/files/search?q=test"
```
...
| `client/src/components/FilePane.tsx` | 検索ロジック更新、props に scenario 追加 |
| `client/src/App.tsx` | FilePane に scenario props を渡す |
...
```typescript
searchFiles: async (scenario: string, query: string): Promise<SearchResult[]> => {
  const res = await fetch(`${baseUrl}/scenarios/${encodeURIComponent(scenario)}/files/search?q=${encodeURIComponent(query)}`);
...
変更内容:
1. `scenario` を props に追加（検索APIに必要）
2. デバウンス検索（300ms）を実装
3. 空クエリ: 既存のローカルフィルタ
...
| パラメータ | 型 | 必須 | 説明 |
|-----------|------|------|------|
| `scenario` | string | Yes | シナリオ名（URLパス） |
...
| フェーズ | ファイル | 変更内容 |
|---------|---------|---------|
| 1 | `server/core/storage.ts` | `searchScenarioFiles` 関数追加 |
| 1 | `server/handlers/control-api.ts` | `/scenarios/:scenario/files/search` エンドポイント追加 |
| 2 | `client/src/types/index.ts` | `SearchResult` 型追加 |
| 2 | `client/src/hooks/useSnaperroAPI.ts` | `searchFiles` 関数追加 |
| 2 | `client/src/components/FilePane.tsx` | 検索ロジック更新、props に scenario 追加 |
| 2 | `client/src/App.tsx` | FilePane に scenario props を渡す |
